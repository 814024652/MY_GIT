...
DEBUG_MODE=""
SERVER_NAME=`grep -v '^\s*#' conf/application.yml | sed '/server-name/!d;s/.*://' | tr -d '\r'`
SERVER_PORT=$1
LOGS_DIR=$DEPLOY_DIR/logs
#LOGS_DIR="logs"
APPLICATION_LAUNCHER=`sed '/server-launcher/!d;s/.*://' conf/application.yml | tr -d '\r'`
...
if  [  -n "$SERVER_PORT" ] ;then
    ...
    SERVER_PORT=`grep -v '^\s*#' conf/application.yml | sed '/server-port/!d;s/.*://' | tr -d '\r'`
	if  [  -n "$SERVER_PORT" ] ;then
	    echo "Server_Port is : $SERVER_PORT"
	else
	    SERVER_PORT="8080"
...
if test -n "/usr/java/jdk1.8.0_144"; then
...
while [ $COUNT -lt 1 ]; do
    sleep 1
    if [ -n "$SERVER_PORT" ]; then
            COUNT=`netstat -an | grep -s -R -n $SERVER_PORT | wc -l`
